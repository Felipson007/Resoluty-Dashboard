...
exports.googleSheetsService = {
     ...
     */
    async logUserAction(action, details, userId = 'frontend-user') {
        try {
     ...
     */
    async logFrontendEvent(event, details) {
        try {
            const timestamp = new Date().toISOString();
            const formattedDate = new Date().toLocaleString('pt-BR');
            // Formatar dados para inserção
    ...
    async getStats() {
        try {
            ...
            const recentUserActions = userActionsData.filter((row) => {
                if (row.length < 5)
                    return false;
                const rowDate = new Date(row[4]); // timestamp está na coluna 5 (índice 4)
                return rowDate > yesterday;
            });
            const recentFrontendEvents = frontendEventsData.filter((row) => {
                if (row.length < 4)
                    return false;
